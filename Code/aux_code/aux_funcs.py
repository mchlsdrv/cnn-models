...
def get_model(net_name, model_type, number_of_classes, crop_size, priors=None):
    ...
    if model_type == 'conv_net':
        ...
        if net_name == 'feature_extractor':
            ...
            assert priors is not None, f'The \'priors\' can not be None!'
...
def get_arg_parcer():
    ...
    parser.add_argument(dest='feature_extractor_train_epochs', type=int, help='Number of epochs to train the feature extractor network')
    parser.add_argument(dest='feature_extractor_train_steps_per_epoch', type=int, help='Number of iterations that will be performed on each epoch for the featrue extractor network')

    # b) Classifier network
    parser.add_argument(dest='classifier_net', type=str, choices=['conv_net', 'res_net_x18', 'res_net_x34'], help='The network which will be built to classify the samples')
    parser.add_argument(dest='number_of_classes', type=int, help='The number of classes to assign to the samples')
    parser.add_argument(dest='classifier_train_epochs', type=int, help='Number of epochs to train the classifier network')
    parser.add_argument(dest='classifier_train_steps_per_epoch', type=int, help='Number of iterations that will be performed on each epoch for the classifier network')
